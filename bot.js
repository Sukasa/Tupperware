...
bot.launch = function () {
	...
	bot.servercfg = require("./servercfg.json");
	...
	files.forEach(file => {
		bot[file.slice(0, -3)] = require("./config/" + file);
	});

	// Load Eris event listeners
	files = fs.readdirSync("./listeners");
	files.forEach(file => {
		bot.on(file.slice(0, -3), (...args) => require("./listeners/" + file)(...args, bot));
	});
...
bot.cmds = {
	...
	form: {
		...
		usage: cfg => ["form[s] <name> <formName> - Switch a " + cfg.lang + "'s form",
		"form[s] <name> add <formName> <formUrl> - Add a new named form to a " + cfg.lang + " (4 max)",
		"form[s] <name> remove <formName> - Remove a named form from a " + cfg.lang + ".  Cannot remove their last form",
			"form[s] <name> rename <oldName> <newName> - Rename a form"],
		aliases: ["forms"],
		permitted: () => true,
		execute: function (msg, args, cfg) {
			let out = "";
